...
import json
from kill_all_env import stop_emulator
...
class TrajectoryRunnerActor:
    def __init__(self, task_info: Dict[str, Any], runner_cfg):
        ...
        self.env_init_timeout = 1200.0  # 环境初始化超时时间
        self.env_init_start_time = None
        self.env_timeout_task = None  # 环境超时监控任务
    ...
    def get_service_id(self) -> Optional[str]:
        ...
        #     return None
            ...
            logger.warning(f"[{self.trace_id}] 环境emulator_id为空 - task_id: {self.task_id}")
            return None
        else:
            logger.info(f"[{self.trace_id}] 成功获取service_id: {self.env.provider.emulator_id} - task_id: {self.task_id}")
    ...
    async def monitor_env_timeout(self):
        ...
        if self.env_init_start_time is None:
            logger.warning(f"[{self.trace_id}] env_init_start_time未设置，无法监控超时 - task_id: {self.task_id}")
            return
        ...
        while True:
            ...
            if elapsed_time > self.env_init_timeout:
                logger.warning(f"[{self.trace_id}] 环境运行超时 - task_id: {self.task_id}, "
                            f"已用时间: {elapsed_time:.2f}s, 超时阈值: {self.env_init_timeout}s")
                try:
                    if self.env is not None:
                        self.env.close()
                        logger.info(f"[{self.trace_id}] 因超时已关闭环境 - task_id: {self.task_id}")
                except Exception as e:
                    logger.error(f"[{self.trace_id}] 超时关闭环境时发生错误 - task_id: {self.task_id}, 错误: {e}")
                finally:
                    self.env = None
                break
    ...
    async def run_episode(self, model_pool, storage, mysql_writer):
        ...
        try:
            ...
            try:
                ...
                if not self.env_init_success:
                    try:
                        ...
                        logger.info(f"[{self.trace_id}] 失败任务信息已保存到: {task_error_path}")
                    except Exception as e:
                        logger.error(f"[{self.trace_id}] 保存失败任务信息失败 - task_id: {self.task_id}, 错误: {e}")
            finally:
            ...
            if self.env is not None:
                try:
                    self.env.close()
                    # print(f"[{self.env.provider.emulator_id}] 环境emulator_id: {self.env.provider.emulator_id}")
                    # stop_emulator(self.env.provider.emulator_id, os.environ.get("OSWORLD_TOKEN"), os.environ.get("OSWORLD_BASE_URL"))
                    logger.info(f"[{self.trace_id}] 环境已关闭 - 环境emulator_id: {self.env.provider.emulator_id} - task_id: {self.task_id}")
                except Exception as e:
        ...
        for attempt in range(max_retries):
            try:
                if env_cfg.env_type == "remote_desktop":
                    ...
                    # os.environ["OSWORLD_BASE_URL"] = 'http://10.1.110.48:50003,http://10.1.110.43:50003'
